...
@RestController
@Slf4j
@RequestMapping(Constants.CR_API_PATH_PREFIX + "import")
public class ImportController {
  ...
  @CustomSecurityAnnotation(target = CONFIG_PROPERTY.SUB_ADMIN_EDIT,
      permission = {SUB_ADMIN_EDIT_VIEW.CR_CAB_USERS, SUB_ADMIN_EDIT_VIEW.PROCESS_MANAGEMENT},
      modulePermission = {"CR_CABUSERS", "CR_PROCESS_MANAGER"}
  )
  ...
  public ResponseEntity<Resource> onImportFile(MultipartFile file,
    ...
    switch (moduleName) {
      case "CR_CABUSERS":
        data = crCabUsersBusiness.importData(file);
        break;

      case "CR_PROCESS_MANAGER":
        data = crManagerProcessBusiness.importData(file);
        break;
