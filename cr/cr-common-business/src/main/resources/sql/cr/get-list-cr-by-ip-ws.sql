WITH t1 AS
  (SELECT *
  FROM open_pm.cr t1
  WHERE t1.CREATED_DATE >= :impactedTimeFrom
  AND t1.CREATED_DATE   <= :impactedTimeTo
  ...
  WHERE t2.ip         in (:ips)
  AND t2.INSERT_TIME >= :impactedTimeFrom
   AND t2.INSERT_TIME <= :impactedTimeTo
  )
...
ON t1.CR_ID   = t8.CR_ID
AND t8.STATUS = 9
LEFT JOIN OPEN_PM.CR_CREATED_FROM_OTHER_SYS t9
ON t1.CR_ID   = t9.CR_ID
where t9.CR_ID is null
