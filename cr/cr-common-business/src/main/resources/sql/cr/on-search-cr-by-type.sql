with temp_cr_his as(
    select distinct cr_id from open_pm.cr_his chs1 where 1 = 1 and chs1.action_code in (:action_reject,:act_close,:act_close_by_appr,:act_close_by_man,:act_close_by_emer)
    )
select cr.title title, cr.cr_id crId,
                              ...
                              else TO_CHAR(usOri.username || ' ('||usOri.mobile||')') end as changeOrginatorName,
                              ...
                              then
                              case
                              when tcr_his.cr_id is not null
--                                   tiennv cap nhat toi uu sql

--                                   exists (select 1 from
--                                   open_pm.cr_his chs1
--                                   where cr.cr_id = chs1.cr_id and chs1.action_code in (:action_reject,:act_close,:act_close_by_appr,:act_close_by_man,:act_close_by_emer))
                                then ''
                              ...
                              left join temp_cr_his tcr_his on cr.cr_id = tcr_his.cr_id
