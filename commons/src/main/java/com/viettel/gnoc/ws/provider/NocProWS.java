...
public class NocProWS {
  ...
  private void createNOCWS() throws Exception {
    String link = "";
    try {
      WSNocprov4Port wsNocprov4Port = SpringApplicationContext.bean(WSNocprov4Port.class);
      link = wsNocprov4Port.NOCPRO_URL;
      username = wsNocprov4Port.userWS;
      password = wsNocprov4Port.passWS;
    } catch (Exception e) {
      log.error(e.getMessage(), e);
    }
    ;
    URL baseUrl;
    baseUrl = GnocWebservice_Service.class.getResource(".");
    URL url = new URL(baseUrl, link);
    logger.info("connect webservice qlts: link=" + link + " begin time: " + new Date());
    GnocWebservice_Service service = new GnocWebservice_Service(url,
        new QName("http://service.nocpro.nms.viettel.com/", "GnocWebservice"));
    port = service.getGnocWebservicePort();
    logger.info("connect webservice qlts success end time:" + new Date());
  }

  public ResponseBO updateCrToFinishImpact(String crId, String changeResponsible) throws Exception {
    ResponseBO resultBO = null;

    try {
      log.info("Call Nocpro WS step 1");
      if (port == null) {
        createNOCWS();
      }
      if (port != null) {
        log.info("Call Nocpro WS step 2");
        resultBO = port.updateCrToFinishImpact(crId, username, password, changeResponsible);
        log.info("Nocpro WS Result : " + resultBO.getDescription());
      }
    } catch (Exception e) {
      log.error(e.getMessage(), e);
      throw e;
