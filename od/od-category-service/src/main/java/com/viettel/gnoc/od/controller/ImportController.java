...
@RestController
@Slf4j
@RequestMapping(Constants.OD_API_PATH_PREFIX + "import")
public class ImportController {
  ...
  public ResponseEntity<Resource> onImportFile(MultipartFile file,
    ...
    switch (moduleName) {
      case "OD_TYPE":
        data = odTypeBusiness.importData(file);
        break;
      case "OD_CONFIG_BUSINESS":
        data = odChangeStatusBusiness.importData(file);
        break;
      case "OD_CONFIG_SCHEDULE_CREATE":
        data = odCfgScheduleCreateBusiness.importData(file, filesAttachment);
        break;
      default:
