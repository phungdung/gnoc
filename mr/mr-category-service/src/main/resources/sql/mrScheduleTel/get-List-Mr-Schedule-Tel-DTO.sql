SELECT  T1.SCHEDULE_ID scheduleId,
        ...
        (CASE WHEN T1.MR_HARD = '1' THEN T1.MR_HARD_CYCLE ELSE T4.CYCLE END) cycle,
        T4.CYCLE_TYPE cycleType,
        (CASE WHEN T4.CYCLE_TYPE = 'M' THEN :cycleTypeM ELSE :cycleTypeD END) cycleTypeName,
...
FROM    MR_SCHEDULE_TEL T1
        LEFT JOIN MR_CAT_MARKET T2 ON T1.MARKET_CODE = T2.MARKET_CODE
        LEFT JOIN MR_CAT_ARRAY T3 ON T1.ARRAY_CODE = T3.ARRAY_CODE
        LEFT JOIN MR_CFG_PROCEDURE_TEL T4 ON T1.PROCEDURE_ID = T4.PROCEDURE_ID
        LEFT JOIN MR_DEVICE T5 ON T1.DEVICE_ID = T5.DEVICE_ID AND T1.DEVICE_CODE = T5.NODE_CODE
        LEFT JOIN (SELECT DISTINCT mr_id, wo_id FROM MR_NODES WHERE wo_id > 0) T8 ON T1.MR_ID = T8.MR_ID
        LEFT JOIN MR_CFG_CR_UNIT_TEL T9 ON T1.MARKET_CODE = T9.MARKET_CODE
        ...
        LEFT JOIN MR_HARD_GROUP_CONFIG T10 ON T1.MARKET_CODE = T10.MARKET_CODE
