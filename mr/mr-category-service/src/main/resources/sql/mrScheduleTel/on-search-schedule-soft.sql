...
SELECT T1.SCHEDULE_ID scheduleId,
  ...
  case when U1.unit_code is null then ''
    else TO_CHAR(U1.unit_code || ' (' || U1.unit_name || ')') end as checkingUnitName,
  case when U2.unit_code is null then ''
    else TO_CHAR(U2.unit_code || ' (' || U2.unit_name || ')') end as implementUnitName
...
ON T1.MR_CONFIRM   = T11.CONFIG_CODE and T11.CONFIG_GROUP='LY_DO_KO_BD'
LEFT JOIN COMMON_GNOC.UNIT U1
ON (T9.CHECKING_UNIT = U1.UNIT_ID AND U1.STATUS = 1)
LEFT JOIN COMMON_GNOC.UNIT U2
ON (T9.IMPLEMENT_UNIT = U2.UNIT_ID AND U2.STATUS = 1)
LEFT JOIN OPEN_PM.CR T12
ON T1.CR_ID   = T12.CR_ID
