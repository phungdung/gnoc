...
SELECT T1.ID id,
  ...
  C.CONFIG_NAME mrConfirmHardDisplay,
  T1.STATUS status,
  T1.NOTES notes,
  T1.SYN_DATE synDate,
...
AND T1.REGION_HARD = T3.REGION
LEFT JOIN COMMON_GNOC.UNIT T4
ON T3.IMPLEMENT_UNIT = T4.UNIT_ID
LEFT JOIN COMMON_GNOC.UNIT T5
ON T3.CHECKING_UNIT = T5.UNIT_ID
LEFT JOIN
  ...
  ) CF ON T1.MARKET_CODE = CF.MARKET_CODE
AND T1.REGION_HARD       = CF.REGION
AND T1.ARRAY_CODE        = CF.ARRAY_CODE
AND T1.DEVICE_TYPE       = CF.DEVICE_TYPE
AND T1.STATION           = CF.STATION_CODE
LEFT JOIN OPEN_PM.MR_CONFIG c
ON T1.MR_CONFIRM_HARD = c.CONFIG_CODE
AND c.CONFIG_GROUP    = 'LY_DO_KO_BD'
LEFT JOIN COMMON_GNOC.UNIT T6
ON t1.BO_UNIT_HARD = T6.UNIT_ID
WHERE 1            = 1
