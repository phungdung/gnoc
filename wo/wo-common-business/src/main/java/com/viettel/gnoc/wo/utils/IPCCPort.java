...
public class IPCCPort {
  ...
  @Autowired
  IPCCPortFactory ipccPortFactory;
  ...
  private void createConnect(String wsLink) throws MalformedURLException, Exception {
    ...
    port = (CallService) ipccPortFactory.init(wsLink).borrowObject();
    System.out.println(
  ...
  public NomalOutput autoCallout(String wsLink, AutoCallOutInput input) {
    try {
      ...
      try {
        if (null != port) {
          ipccPortFactory.init(wsLink).returnObject(port);
        }
      } catch (Exception e) {
        log.error(e.getMessage(), e);
      }
