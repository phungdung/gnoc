...
public class IPCCVTZPort {
  ...
  @Autowired
  IPCCVTZPortFactory ipccvtzPortFactory;
  ...
  private void createConnect(String wsLink) throws MalformedURLException, Exception {
    System.out.println("Start create connect IPCCVTZ WS ");
    long startTime = Calendar.getInstance().getTimeInMillis();
    port = (CallService) ipccvtzPortFactory.init(wsLink).borrowObject();
    System.out.println(
        "End create connect IPCCVTZ WS: " + (Calendar.getInstance().getTimeInMillis() - startTime));
  ...
  public NomalOutput autoCallout(String wsLink, AutoCallOutInput input) {
    try {
      ...
      try {
        if (null != port) {
          ipccvtzPortFactory.init(wsLink).returnObject(port);
        }
      } catch (Exception e) {
        log.error(e.getMessage(), e);
      }
