...
public class IPCCVTPPort {
  ...
  @Autowired
  IPCCVTPPortFactory ipccvtpPortFactory;
  ...
  private void createConnect(String wsLink) throws MalformedURLException, Exception {
    System.out.println("Start create connect IPCCVTP WS ");
    long startTime = Calendar.getInstance().getTimeInMillis();
    port = (CallService) ipccvtpPortFactory.init(wsLink).borrowObject();
    System.out.println(
        "End create connect IPCCVTP WS: " + (Calendar.getInstance().getTimeInMillis() - startTime));
  ...
  public NomalOutput autoCallout(String wsLink, AutoCallOutInput input) {
    try {
      ...
      try {
        if (null != port) {
          ipccvtpPortFactory.init(wsLink).returnObject(port);
        }
      } catch (Exception e) {
        log.error(e.getMessage(), e);
      }
