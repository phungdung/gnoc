...
w.wo_id woId,
  ...
  (SELECT p.wo_code FROM wfm.wo p WHERE p.WO_ID = w.PARENT_ID) parentName,
...
(select count(wo_worklog_id) from wfm.WO_WORKLOG where wo_id = w.wo_id) hasWorklog,
rank()
    over (order by w.create_date desc ) indexRow
FROM wfm.wo w
